...
void stampaarray(float s[]){ //MADE BY CESARANO
...
void setConstantVelocityTo(float point[], float k){ // MADE BY ALL
	    float myState[12];
...
void mathVecMult (float *v, float *a, float k, int dim){//MADE BY RICCIO-CESARANO
    int i;
    for (i=0; i<dim; i++){
        v[i]=a[i]*k;
    }
...
bool getColor(){ //MADE BY RICCIO-CESARANO
