...
void init(){
    attitudeToEarth=false;
    // DEBUG(("%d",attitudeToEarth));
    //srand(time(NULL));
   // (rand() % 2)==0?SET=-1:SET=1;
    
    /*for(int i=0;i<9;i++)//get all position of all item
        game.getItemLoc(allItem[i],i);*/
        
    
    Time = 0;
    Phase = 1;
    DEBUG(("\n Greetings from the school Augusto Righi, Naples (Italy) XD"));
...
void loop()
{
    ...
    if(Time<1)
        Color = ((myState[0] < 0) ? 0 : 1); //use this for index in matrix,
                                            //then, go to correct point, indifferently
   ...
   {
        float Strategy[]={-0.32f,0.40f,myState[2]-0.05};//point for remain inside the bounds
    ...
    else
    {
        if(Phase==1)
        {
           ...
           velocity = (((getDistance(myState,whereToGo)*0.68f)/ 0.0607148161220323f)/100.0f); 
           //+0.02 = +4 sec  0.04624f
        }//0.0457148161220323f
        else
        {
