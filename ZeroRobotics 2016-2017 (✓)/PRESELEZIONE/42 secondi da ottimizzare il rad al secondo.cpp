...
void loop(){
    ...
    if(actualProcedure == 1){
        goOnCircle(0.22,true,-0.5* (isBlue?1:-1) ,-0.25* (isBlue?1:-1),false);//0.25 raggio
        float whereRotate[3] = {-0.5* (isBlue?1:-1), 0.0, 0.0f };
        //float whereToGo[3] = {-0.41* (isBlue?1:-1), -0.02* (isBlue?1:-1), 0.0f };
        if(getDistance(myState,whereRotate)<0.05){ targetRate[2] = (62*PI/180) * -1;} //58.2958
        if(getDistance(myState,whereRotate)<0.05){ actualProcedure = 2;}
    }
    if(actualProcedure==2){
       // float whereToGo[3] = {-0.082f* (isBlue?1:-1), 0.0f,  -0.5f* (isBlue?1:-1)};
        float whereToGo[3] = {-0.11f* (isBlue?1:-1), -0.0431f * (isBlue?1:-1),  -0.543f* (isBlue?1:-1)};
        float trueWhereToGo[3] = {-0.0f* (isBlue?1:-1), -0.0f,  -0.5f* (isBlue?1:-1)};
        float distanceToWhereGo = getDistance(myState, whereToGo);
        float kVel = 0.025,velocity = (((distanceToWhereGo*0.68f)/kVel)/100.0f);
        setConstantVelocityTo(whereToGo, velocity);
        if(getDistance(myState,trueWhereToGo)<0.04f){ actualProcedure = 3;}
        //goOnCircle(0.30,0.428,true,-0.46* (isBlue?1:-1) ,-0.46* (isBlue?1:-1),true);//0.25 raggio
    }
    if(actualProcedure == 3){
        //DEBUG(("CI SONO"));
        float whereToGo[3] = {-0.0f* (isBlue?1:-1), -0.0f,  -0.5f* (isBlue?1:-1)};
        targetRate[2] = (57.5*PI/180) * -1;
