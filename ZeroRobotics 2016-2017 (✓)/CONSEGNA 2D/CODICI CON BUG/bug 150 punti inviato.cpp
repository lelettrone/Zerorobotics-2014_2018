...
void loop(){
    ...
    //DEBUG(("CIAOOOOOO:%d",id));
   DEBUG(("RAMM A BORZ, RAMM A BORZZZ"));
    ...
    if(SPSHeld==3){
        whereToGo[2]=0;
        whereToGo[0] = ((Color==0) ? 15.0f : -15.0f);
        whereToGo[1] = ((Color==0) ? 100.0f : -100.0f);
        if(myState[1]>1.9f&&!Color)
            game.dropSPS();
        if(myState[1]<-1.9f&&Color)
            game.dropSPS();
    ...
    if(SPSHeld==2){
        whereToGo[0] = ((Color==0) ? 6.0f : -6.0f);
        whereToGo[1] = ((Color==0) ? -10.0f : 10.0f);
        if(myState[1]<-2.9f&&!Color){
            game.dropSPS();
            whereToGo[0] =-100.0;
            whereToGo[1] = ((Color==0) ? -3.0f : 3.0f);
        }
        if(myState[1]>2.9f&&Color){
            game.dropSPS();
            whereToGo[0] =100.0;
