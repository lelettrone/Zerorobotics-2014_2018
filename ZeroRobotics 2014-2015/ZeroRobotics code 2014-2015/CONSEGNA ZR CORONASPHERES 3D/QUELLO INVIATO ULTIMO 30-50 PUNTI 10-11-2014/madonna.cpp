...
bool alignPOI()
{
    ...
    angleMePoi=getAngleBetween(myState, itemPos[0], itemPos[0], zero);
    
    //Angle between ourAttitude-POI-idealAttitude
    mathVecSubtract(rightAttitude,myState,itemPos[0],3);
    //angleAttitude=getAngleBetweenVector(myAtt,rightAttitude);
    angleAttitude=mathVecInner(rightAttitude,myAtt,3);
    DEBUG(("\n ANGOLO TORSIONE: %f", angleAttitude));
...
float getAngleBetweenVector(float vectorA[3], float vectorB[3])
{
    ...
    DEBUG(("\n ANGOLO PRIMA: %f",angleBetween));
    
    angleBetween=mathVecInner(vectorA, vectorB, 3);
    DEBUG(("\n ANGOLO DOPO: %f",angleBetween));
...
*   LAST CHANGES BY RUSSO 14.16 - 26/10
    -   Changed the type of attitude-orientation:
        now we orient the camera towards the centre if Step=0, (pictures in inner zone)
        otherwise we use the previous strategy
*/

#define upl 0.56    //radius upload zone
#define out 0.50    //radius outer zone
#define inn 0.40    //radius inner zone
float zero[3];
...
void loop()
{
    ...
    switch(Phase)
    {
        ...
        case 1:
            ...
            if(nPhoto==0)
            {
                mathVecSubtract(vectorBetween,zero,myState,3);  //Attitude towards centre.
            }
            else
            {
                mathVecSubtract(vectorBetween,itemPos[Step],myState,3); //Attitude towards POI
            }
        ...
        if(timeToTurnOn>5)
        {
            if(((api.getTime()-waitTime)>3)&&alignPOI()&&((!((solarFlare!=-1)&&(solarFlare<8)))||(getRTNR()<17)))     
            {
                    waitTime=api.getTime();
                    game.takePic(idPOI);
        ...
        {
            takenPos=false;
            distStop=130*getVelocity()*getVelocity();
            if (getDistance(myState,itemSetPos[Step])>distStop)
                {
                    setConstantVelocityTo(itemSetPos[Step],0.055);
                }
            else
            {
                api.setPositionTarget(itemSetPos[Step]);
        ...
        else
        {
            ...
            if (getDistance(myState,itemSetPos[Step])>distStop)
                {
                    setConstantVelocityTo(itemSetPos[Step],0.6);
                }
            else
            {
                api.setPositionTarget(itemSetPos[Step]);
